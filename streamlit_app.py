...
st.set_page_config(page_title= "Youtube Data Harvesting and Warehousing | By Asvin S",
                  
                   layout= "wide",
                   initial_sidebar_state= "expanded",
                   menu_items={'About': """# This app is created by Asvin S!*"""})
...
api_key = "AIzaSyAGGk3CwjsHCaCxpSkWkIKyFXaE_ASJmdg"
...
def insert_into_videos():
    ...
    query1 = """
    INSERT INTO videos ( Channel_name, Channel_id, Video_id, Title, Thumbnail,Duration, Views, Likes, Comments, Favorite_count, Definition, Caption_status)
    VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)
    """
#tag,
    for doc in collectionss.find({"Channel_name": user_inp}, {"_id": 0}):
        values = (
           ...
           # doc.get("Tags"),
            ...
            doc.get("Likes"),
            doc.get("Comments"),
            doc.get("Favorite_count"),
            doc.get("Definition"),
            doc.get("Caption_status")
        )
        print(values)
...
def insert_into_comments():
    ...
    query2 = """
    INSERT INTO comments (Comment_id, Video_id, Comment_text, Comment_author, Like_count, Reply_count)
    VALUES (%s, %s, %s, %s, %s, %s)
    """

    for vid in collections1.find({"Channel_name" : user_inp},{'_id' : 0}):
            for comment in collections2.find({'Video_id': vid['Video_id']},{'_id' : 0}):
                        values = (
                              comment.get("Comment_id"),
                              comment.get("Video_id"),
                               comment.get("Comment_text"),
                               comment.get("Comment_author"),
                              # comment.get("Comment_posted_date"),
...
if selected == "Extract and Transform":
    ...
    with tab1:
        ...
        ch_id = st.text_input("Hint : Go to channel's home page >> Right click >> View page source >> Find channel_id").split(',')
...
if selected == "View":
    ...
    elif questions == '8. What is the average duration of all videos in each channel, and what are their corresponding channel names?':
        mycursor.execute("""SELECT channel_name, 
